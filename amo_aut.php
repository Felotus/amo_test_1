...
class amo_user {
	...
	private function data_clean($value = ""){
    	$value = trim($value);
	...
	private function req_curl($links, $data = NULL){
		...
		curl_setopt($curl, CURLOPT_URL, $this->link.$links);
	...
	public function create_multitext($name, array $enums_val, $elem_type = 1){
		$fields['add'] = [
			[
				'name' => $name,
				'field_type'=> 5,
...
$amo_us = new amo_user($config['akk'], $config['mail'], $config['link'], $config['hash']);
$amo_us->amo_auth();
